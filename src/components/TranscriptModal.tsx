...
export function TranscriptModal({
  ...
  const [viewMode, setViewMode] = useState<'transcript' | 'scorecard'>('transcript');
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {selectedTranscript && (
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <h3 className="text-lg font-semibold text-white">
                      {viewMode === 'transcript' ? 'Call Transcript' : 'Call Scorecard'}
                    </h3>
                    {/* View Mode Toggle */}
                    <div className="flex items-center bg-[#1a1f2e] rounded-lg p-0.5">
                      <button
                      ...
                      <button
                        onClick={() => {
                          if (!callAnalysis && !analyzingCall) {
                            analyzeCall();
                          } else {
                            setViewMode('scorecard');
                          }
                        }}
                        disabled={analyzingCall || !selectedTranscript?.conversation?.length}
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                          viewMode === 'scorecard'
                            ? 'bg-blue-500/20 text-blue-300'
                            : 'text-gray-400 hover:text-gray-300'
                        } ${analyzingCall ? 'opacity-50 cursor-wait' : ''} ${!selectedTranscript?.conversation?.length ? 'opacity-30 cursor-not-allowed' : ''}`}
                      >
                        {analyzingCall ? (
                          <div className="animate-spin rounded-full h-3.5 w-3.5 border border-blue-500/20 border-t-blue-500" />
                        ) : (
              ...
              {/* Content Area - switches between Transcript and Scorecard */}
              {viewMode === 'scorecard' ? (
